...
import { MessageSquare, Plus, Loader2, Pencil } from 'lucide-react';
...
export default function EnquiriesPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', email: '', message: '', status: 'new' });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof form) => {
      if (editing) {
        ...
        const { error } = await supabase.from('enquiries').insert({ ...data, created_by: currentUser?.id });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  function openAdd() {
    setEditing(null);
    setForm({ name: '', phone: '', email: '', message: '', status: 'new' });
    setDialogOpen(true);
  }

  function openEdit(e: any) {
    setEditing(e);
    setForm({ name: e.name, phone: e.phone || '', email: e.email || '', message: e.message || '', status: e.status });
    setDialogOpen(true);
  }
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-display font-bold tracking-tight">Enquiries</h1>
          <Badge variant="secondary">{enquiries.length}</Badge>
        </div>
        <Button onClick={openAdd}><Plus className="h-4 w-4" /> New Enquiry</Button>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr><td colSpan={5} className="text-center py-8 text-muted-foreground">No enquiries</td></tr>
                ) : enquiries.map((e: any) => (
                  <tr key={e.id} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="p-4 hidden sm:table-cell text-muted-foreground">{e.phone || '—'}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(form); }} className="space-y-4">
            ...
            <DialogFooter><Button type="submit" disabled={saveMutation.isPending}>{saveMutation.isPending && <Loader2 className="h-4 w-4 animate-spin" />}{editing ? 'Update' : 'Add'}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </div>
